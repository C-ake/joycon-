...
struct buttonData{
	...
	bool x = false;
	bool y = false;
	bool special = false;			//Home or Screenshot
	bool symbol = false;			//Plus or Minus Button

	/* Triggers */
	bool lTrigger = false;		//Left Side Trigger
	bool rTrigger = false;		//Right Side Trigger
	bool kTrigger = false;		//Small Trigger
	bool zTrigger = false;		//Large Trigger

	/* Joysticks */
	double xJoy = 0;			//Joystick X-Offset
	double yJoy = 0;			//Joystick Y-Offset
	bool joyclick = false;
};

/* Overloaded ostream operator for buttonData */
std::ostream & operator << (std::ostream &out, const buttonData &d){
		out << "A" << d.a <<" ";
		out << "B" << d.b <<" ";
		out << "X" << d.x <<" ";
		out << "Y" << d.y <<" ";
...
to_int(const_iterator it_begin, const_iterator it_end, bool bigEndian = true) {
	...
	unsigned int length = std::distance(it_begin, it_end);
